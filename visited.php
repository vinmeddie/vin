...
<?php include('header.php')?>
  ...
  <div class="content">
		...
		<?php
		//get visitors
$nn=mysql_query("SELECT * FROM pofilevisitors WHERE Visitor='".$_SESSION['uid']."'") or die(mysql_error());
$p=mysql_num_rows($nn);
if($p=='1')
{
$mm=$p." Visitor";
...
//end get visitors
	...
	<hr>
		...
		$sql = mysql_query("SELECT * FROM pofilevisitors WHERE Visitor='".$_SESSION['uid']."'"); 
$i = 0;
...
while($row = mysql_fetch_array($sql)){ 
	...
	$sql3 = mysql_query("SELECT * FROM users WHERE id='".$row["visited"]."' ORDER BY rand()");
